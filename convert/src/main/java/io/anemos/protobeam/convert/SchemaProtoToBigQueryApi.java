...
public class SchemaProtoToBigQueryApi {
  ...
  public TimePartitioning getTimePartitioning(Descriptors.Descriptor descriptor) {
    com.google.api.services.bigquery.model.TimePartitioning timePartitioning =
        modelConvert.getTimePartitioning(descriptor);
  ...
  private Field convertField(TableFieldSchema tableFieldSchema) {
    ...
    if (fieldType.equals(LegacySQLTypeName.RECORD)) {
      ...
      fieldBuilder =
          Field.newBuilder(
              tableFieldSchema.getName(), extractFieldType(tableFieldSchema), fieldList);
    } else {
      fieldBuilder =
          Field.newBuilder(tableFieldSchema.getName(), extractFieldType(tableFieldSchema));
    }
    ...
    if ((tableFieldSchema.getDescription() != null)
        && (tableFieldSchema.getDescription().length() > 0)) {
    ...
    switch (tableFieldSchema.getType()) {
      ...
      default:
        throw new RuntimeException(
            "Cannot extractFieldType TableFieldSchema type "
