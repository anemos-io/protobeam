...
public class SchemaProtoContext {
  ...
  public boolean flatten(Descriptors.FieldDescriptor fieldDescriptor) {
    ...
    if (fieldDescriptor.getOptions().hasExtension(Options.fieldRewrite)) {
      Descriptors.FieldDescriptor flattenFieldDescriptor =
          Rewrite.FieldRewriteRule.getDescriptor().findFieldByName("flatten");
      return (Boolean)
          fieldDescriptor
              .getOptions()
              .getExtension(Options.fieldRewrite)
              .getField(flattenFieldDescriptor);
    }
    return false;
  }
  ...
  public boolean hasRenameTo(Descriptors.FieldDescriptor fieldDescriptor) {
    if (fieldDescriptor.getOptions().hasExtension(Options.fieldRewrite)) {
      Descriptors.FieldDescriptor toRenameDescriptor =
          Rewrite.FieldRewriteRule.getDescriptor().findFieldByName("rename_to");
      return fieldDescriptor
          .getOptions()
          .getExtension(Options.fieldRewrite)
          .hasField(toRenameDescriptor);
    }
    return false;
  }
  ...
  public boolean hasBqTimePartitioningField(Descriptors.Descriptor messageDescriptor) {
    if (messageDescriptor.getOptions().hasExtension(Options.messageBigquery)) {
      Bigquery.BigQueryMessageOptions bigQueryMessageOptions =
          messageDescriptor.getOptions().getExtension(Options.messageBigquery);
      return bigQueryMessageOptions.hasField(
          bigQueryMessageOptions.getDescriptorForType().findFieldByName("time_partitioning_field"));
    }
    return false;
  }
  ...
  public String getBqTimePartitioningField(Descriptors.Descriptor messageDescriptor) {
    Bigquery.BigQueryMessageOptions bigQueryMessageOptions =
        messageDescriptor.getOptions().getExtension(Options.messageBigquery);
    return (String)
        bigQueryMessageOptions.getField(
            bigQueryMessageOptions
                .getDescriptorForType()
                .findFieldByName("time_partitioning_field"));
  }
  ...
  public boolean hasNonDefaultTimePartitioningTruncate(Descriptors.Descriptor messageDescriptor) {
    if (messageDescriptor.getOptions().hasExtension(Options.messageBigquery)) {
      Bigquery.BigQueryMessageOptions bigQueryMessageOptions =
          messageDescriptor.getOptions().getExtension(Options.messageBigquery);
      boolean hasField =
          bigQueryMessageOptions.hasField(
              bigQueryMessageOptions
                  .getDescriptorForType()
                  .findFieldByName("add_time_partitioning_truncate_column"));
      if (hasField) {
        Bigquery.BigQueryMessageOptions.AddTimePartitioningTruncateColumn truncate =
            bigQueryMessageOptions.getAddTimePartitioningTruncateColumn();
  ...
  public Bigquery.BigQueryMessageOptions.AddTimePartitioningTruncateColumn
      getTimePartitioningTruncate(Descriptors.Descriptor messageDescriptor) {
    Bigquery.BigQueryMessageOptions bigQueryMessageOptions =
        messageDescriptor.getOptions().getExtension(Options.messageBigquery);
  ...
  public String getTimePartitionColumnName(Descriptors.Descriptor messageDescriptor) {
    ...
    Descriptors.FieldDescriptor sourceFieldDescriptor =
        messageDescriptor.findFieldByName(getBqTimePartitioningField(messageDescriptor));

    if (bigQueryMessageOptions.hasField(
        bigQueryMessageOptions
            .getDescriptorForType()
            .findFieldByName("time_partitioning_truncate_column_name"))) {
    ...
    Bigquery.BigQueryMessageOptions.AddTimePartitioningTruncateColumn truncateField =
        bigQueryMessageOptions.getAddTimePartitioningTruncateColumn();
