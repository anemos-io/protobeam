...
public class SchemaProtoToBeamSQL {
  ...
  public <T extends Message> Schema getSchema(Class<T> messageClass) {
    ...
    try {
      ...
      throw new RuntimeException(
          "ProtoBeam: Unable to get descriptor from Message. Maybe the class is not a Message?", e);
    }
  ...
  private Schema.FieldType extractFieldType(Descriptors.FieldDescriptor fieldDescriptor) {
    ...
    if (context.isTimestamp(fieldDescriptor)) return Schema.FieldType.DATETIME;
    if (context.isDecimal(fieldDescriptor)) return Schema.FieldType.DECIMAL;
